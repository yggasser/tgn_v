...
function pastelColorFromId(id,{hueShift=0,sat=55,light=82}={}){
  const h=(hash32(id)+hueShift)%360;
  return `hsl(${h}, ${sat}%, ${light}%)`;
}


const TERRITORY_ORDER=["districts","prom","micro","parks","cem"];

function territoryKeyForFeature(feature){
  const cats=new Set(featureCategories(feature));
  for(const key of TERRITORY_ORDER){
    const catNorm=norm(state.territories[key].cat);
    if(cats.has(catNorm)) return key;
  }
  return null;
}

function wantTerritoryNow(key){
  const t=state.territories[key];
  if(!t || !t.show || !state.map) return false;
  return state.map.getZoom() <= t.maxZoom;
}

function wantAnyTerritoriesNow(){
  return TERRITORY_ORDER.some(k=>wantTerritoryNow(k));
}

function territoryStyle(key,feature){
  const t=state.territories[key];
  const p=(feature && feature.properties) || {};
  const id=p.id ?? p.title ?? "";
  const fill=pastelColorFromId(String(id),{hueShift:t.hueShift,sat:t.sat,light:t.light});
  return {stroke:false,weight:0,opacity:0,color:fill,fillColor:fill,fillOpacity:t.opacity,interactive:false};
}

async function setTerritoryOpacity(key,target,{durationMs=300}={}){
  const t=state.territories[key];
  if(!t) return;
  const start=Number(t.opacity);
  const diff=target-start;
  if(Math.abs(diff)<0.001){
    t.opacity=target;
    if(t.layer) t.layer.setStyle({fillOpacity:target});
    if(key==="districts" && t.labelLayer){ try{ t.labelLayer.eachLayer(mk=>{ if(mk && mk._icon) mk._icon.style.opacity=String(target); }); }catch(e){} }
    return;
  }
  ...
  return new Promise((resolve)=>{
    const step=(now)=>{
      ...
      t.opacity=val;
      if(t.layer) t.layer.setStyle({fillOpacity:val});
      
      // sync district label opacity with fillOpacity
      if(key==="districts" && t.labelLayer){
        try{
          t.labelLayer.eachLayer(mk=>{ if(mk && mk._icon) mk._icon.style.opacity=String(val); });
        }catch(e){}
      }
...
async function ensureTerritoryLayer(key,features){
  const t=state.territories[key];
  if(!t || !state.map) return;
  if(!state.map.getPane(t.pane)){
    const pane=state.map.createPane(t.pane);
    pane.style.zIndex=String(t.zIndex);
    pane.style.pointerEvents="none";
  }
  const fc={type:"FeatureCollection",features:features||[]};
  if(t.layer) t.layer.remove();
  t.layer=L.geoJSON(fc,{pane:t.pane,style:(f)=>territoryStyle(key,f)});
  t.layer.addTo(state.map);
}

function districtLabelText(feature){
  const p=(feature && feature.properties) || {};
  const t = (p.title || p.name || p.id || "").toString();
  return t.replace(/_/g," ").trim();
}

function districtLabelLatLng(feature){
  const p=(feature && feature.properties) || {};
  const b=p.bbox;
  if(b && b.length===4){
    ...
    if(Number.isFinite(w)&&Number.isFinite(s)&&Number.isFinite(e)&&Number.isFinite(n)){
      return L.latLng((s+n)/2,(w+e)/2);
    }
  }
  // fallback: try polygon centroid (rough)
  const g=feature && feature.geometry;
  try{
    if(g && g.type==="Polygon" && g.coordinates && g.coordinates[0] && g.coordinates[0].length){
      const ring=g.coordinates[0];
      let sx=0, sy=0, c=0;
      for(const pt of ring){
        if(pt && pt.length>=2){
          sx += Number(pt[0]); sy += Number(pt[1]); c += 1;
        }
      }
      if(c>0){
        return L.latLng(sy/c, sx/c);
      }
    }
  }catch(e){}
  return null;
...
async function ensureDistrictLabels(features){
  const t=state.territories && state.territories.districts;
  if(!t || !state.map) return;
  if(!t.labels) return;

  if(!state.map.getPane(t.labelPane)){
    const pane=state.map.createPane(t.labelPane);
    pane.style.zIndex=String(t.labelZIndex || 260);
    pane.style.pointerEvents="none";
  }
  ...
  for(const f of (features||[])){
    const text=districtLabelText(f);
    const ll=districtLabelLatLng(f);
    if(!text || !ll) continue;

    const html = `<div style="
      font: 700 12px/1.1 system-ui, -apple-system, Segoe UI, Roboto, Arial;
      color: rgba(40,50,70,.92);
      text-shadow: 0 1px 0 rgba(255,255,255,.9), 0 2px 6px rgba(0,0,0,.15);
      letter-spacing: .2px;
      white-space: nowrap;
      opacity: ${t.opacity};
      pointer-events:none;
    ...
    const icon=L.divIcon({className:"districtLabelIcon",html,iconSize:null});
    const mk=L.marker(ll,{icon,interactive:false,pane:t.labelPane});
    markers.push(mk);
  }

  t.labelLayer=L.layerGroup(markers);
  t.labelLayer.addTo(state.map);
}

async function hideDistrictLabels({animate=true}={}){
  const t=state.territories && state.territories.districts;
  if(!t || !t.labelLayer) return;
  if(animate){
    // fade out quickly
    try{
      t.labelLayer.eachLayer(mk=>{ if(mk && mk._icon){ mk._icon.style.transition="opacity 220ms ease"; mk._icon.style.opacity="0"; } });
    }catch(e){}
    await new Promise(r=>setTimeout(r,240));
  }
  if(t.labelLayer){ t.labelLayer.remove(); t.labelLayer=null; }
  // reset opacity for next show
  t.opacity=t.baseOpacity;
}


async function hideTerritory(key,{animate=true}={}){
  const t=state.territories[key];
  if(!t || !t.layer) return;
  if(animate) await setTerritoryOpacity(key,0,{durationMs:280});
  if(t.layer){ t.layer.remove(); t.layer=null; }
  t.opacity=t.baseOpacity;
}
...
function setDistrictOpacity(target,{durationMs=350}={}){
  target = Math.max(0, Math.min(1, Number(target)||0));
  const start = Number(state.district.opacity);
  const diff = target - start;
  if(Math.abs(diff) < 0.001){
    state.district.opacity = target;
    if(state.districtLayer) state.districtLayer.setStyle({fillOpacity: target});
  ...
  return new Promise((resolve)=>{
    const step = (t)=>{
      ...
      const val = start + diff * e;
      state.district.opacity = val;
      if(state.districtLayer) state.districtLayer.setStyle({fillOpacity: val});
      if(k < 1) requestAnimationFrame(step);
      else resolve();
    };
    requestAnimationFrame(step);
...
function catGroup(cat){
  ...
  if(["районы","микрорайоны","промзоны","кладбища","парки и скверы"].includes(c)) return {name:"Территории",order:0};
  if(/дом|строение|здание|корпус|общежит/i.test(c)) return {name:"Здания",order:1};
  if(/магазин|кафе|ресторан|столов|школ|детсад|садик|больниц|поликлин|аптек|банк|почт|театр|музей|клуб|офис/i.test(c)) return {name:"Заведения",order:2};
  if(/памятник|культурного наследия|охраняем|истор/i.test(c)) return {name:"Статус и охрана",order:3};
  if(/ворота|парадн|цокол|окн|арка|фасад/i.test(c)) return {name:"Детали и элементы",order:4};
  return {name:"Прочее",order:9};
}
...
function renderCats(){
  ...
  const all=state.cats.filter(c=>!isTimeCategory(c.category));
  const visible=all.filter(c=>!f||c.category.toLowerCase().includes(f));
  const catsShown=all.length;
  ...
  // group + sort inside groups by count desc
  const grouped = {};
  for(const c of visible){
    const g=catGroup(c.category);
    grouped[g.name] = grouped[g.name] || {order:g.order, items:[]};
    grouped[g.name].items.push(c);
  }
  const groups = Object.entries(grouped)
    .map(([name,v])=>({name,order:v.order,items:v.items}))
    .sort((a,b)=>a.order-b.order || a.name.localeCompare(b.name));

  for(const g of groups){
    // header
    const h=document.createElement("div");
    h.className="catGroupHeader";
    h.style.cssText="margin:10px 0 6px;font-weight:700;font-size:12px;color:#2f3d52;opacity:.9;";
    h.textContent=g.name;
    wrap.appendChild(h);

    g.items.sort((a,b)=> (b.count-a.count) || a.category.localeCompare(b.category));
...
async function reloadObjects(){
  const wantObjects = (state.selectedCats.size>0) || yearFilterActive() || !!state.q.trim();
  const wantTerritories = wantAnyTerritoriesNow();
if(!wantObjects && !wantTerritories){
    if(state.layer){state.layer.remove(); state.layer=null;}
    for(const k of TERRITORY_ORDER){ await hideTerritory(k,{animate:true}); }
    await hideDistrictLabels({animate:true});
    setShownCount(0);
    setStatus("Select categories (or zoom out to see territories)…");
    return;
  }
  ...
  try{
    ...
    // 1) Территории (авто-слои: районы/промзоны/микрорайоны/парки/кладбища)
    const terrAssigned = new Set();
    const terrCounts = {};
    for(const key of TERRITORY_ORDER){
      terrCounts[key]=0;
      if(!wantTerritoryNow(key)){
        await hideTerritory(key,{animate:true});
                if(key==="districts"){ await hideDistrictLabels({animate:true}); }
continue;
      }
      const catNorm = norm(state.territories[key].cat);
      const terrFeats = feats.filter(f=>{
        const p=(f && f.properties) || {};
        const id=String(p.id||"");
        if(!id || terrAssigned.has(id)) return false;
        const setCats=new Set(featureCategories(f));
        if(setCats.has(catNorm)){
          terrAssigned.add(id);
          return true;
        }
        return false;
      });
      terrCounts[key]=terrFeats.length;
      if(terrFeats.length){
        // reset base opacity and show with fade-in
        state.territories[key].opacity = state.territories[key].baseOpacity;
        await ensureTerritoryLayer(key, terrFeats);
        await setTerritoryOpacity(key, state.territories[key].baseOpacity, {durationMs:240});
      
        if(key==="districts"){ await ensureDistrictLabels(terrFeats); }
}else{
        await hideTerritory(key,{animate:true});
      }
    }

    // 2) Обычные объекты (по выбранным категориям/годам)

    let objFeats = feats.filter(f=>!terrAssigned.has(String(((f&&f.properties)||{}).id||"")));
    if(wantObjects){
      if(state.selectedCats.size>0) objFeats = objFeats.filter(featureHasAnySelectedCategory);
      if(state.year.min!=null && state.year.max!=null){
        const a=state.year.from??state.year.min; const b=state.year.to??state.year.max;
        objFeats=objFeats.filter(f=>{const y=getFeatureYear(f); if(y==null) return !!state.year.includeUnknown; return y>=a && y<=b;});
      }
    }else{
      objFeats = [];
    }

    const out={type:"FeatureCollection",features:objFeats};
    ...
    const shown = (out.features||[]).length + Object.values(terrCounts).reduce((a,b)=>a+(b||0),0);
    setShownCount(shown);
    setStatus(`In view: ${shown}`);
  }catch(e){
...
(function ensureTerritoryToggleBox(){
  const hint=document.getElementById("catsHint");
  const parent=(hint && hint.parentNode) || document.getElementById("sidebar") || document.body;
  if(document.getElementById("territoryToggleBox")) return;

  const box=document.createElement("div");
  box.id="territoryToggleBox";
  ...
  box.innerHTML = `
    <div style="font-weight:700;margin-bottom:6px;">Территории (авто по масштабу)</div>
    <label style="display:flex;align-items:center;gap:8px;margin:4px 0;"><input type="checkbox" data-key="districts" checked>Районы</label>
    <label style="display:flex;align-items:center;gap:8px;margin:4px 0;"><input type="checkbox" data-key="micro" checked>Микрорайоны / кварталы / посёлки</label>
    <label style="display:flex;align-items:center;gap:8px;margin:4px 0;"><input type="checkbox" data-key="prom" checked>Промзоны / крупные предприятия</label>
    <label style="display:flex;align-items:center;gap:8px;margin:4px 0;"><input type="checkbox" data-key="parks" checked>Парки и скверы</label>
    <label style="display:flex;align-items:center;gap:8px;margin:4px 0;"><input type="checkbox" data-key="cem" checked>Кладбища</label>
    <div style="margin-top:6px;font-size:11.5px;color:#667;">Показываются только на отдалении и растворяются при приближении.</div>
  `;
  parent.insertBefore(box, parent.firstChild);

  box.querySelectorAll('input[type="checkbox"][data-key]').forEach(cb=>{
    const key=cb.dataset.key;
    cb.checked = !!(state.territories[key] && state.territories[key].show);
    cb.addEventListener("change",()=>{ if(state.territories[key]){ state.territories[key].show=!!cb.checked; scheduleReload(); } });
  });
})();
  setShownCount(0);
